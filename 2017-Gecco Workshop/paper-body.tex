...
of PSO and GA EvoWorkers was used to run several benchmark
functions found in the COCO platform. After the execution,
a script processed the logs and generated the files needed
by the COCO platform post-processing scripts. 
...
Using this data, we consider the result after each 
iteration as a single function evaluation and add the 
number of function evaluations in the iteration to the 
sequential count. Is important to
...
the maximum number of function evaluations in an 
iteration was 200. 
...
containers and running the benchmark. As a case study
functions F1 to F3 were tested with 15 instances and
in dimensions 2, 3, 5, 10, 20, and 40.

The maximum
number of function evaluations was set at $10^5*dim$.
