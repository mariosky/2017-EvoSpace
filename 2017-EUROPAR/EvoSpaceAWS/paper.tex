...
each worker.  In this paper we present a method inspired by a strategy proposed by Gong and Fukunaga for the
...
smartphones and tablets, as well as massive data centers \cite{agajaj,falta}. 
...
self-adaptive parameters \cite{pellerin2004self}, hybrid approaches \cite{None} and so on.
...
\begin{figure*}[h!t]
    ...
    homogeneous configuration as a boxplot to the left of the $x$ axis
...
solution. In this case, the heterogenous/random configuration
...
more on the explorative part than the exploitative part. The
heterogeneous worker solution, however, seems to lean more on
exploitation by keeping high diversity, something that might not be
