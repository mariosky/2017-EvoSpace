...
Table \ref{tab:params}. Additionally, for the binary OneMax problem bit-flip mutation is used and for the real-valued problems uniform 
with an 
independent probability (ind prob) for each attribute to be flipped of $0.05$, 
and a tournament selection of size 3. 
 we use ind_prob mutation and XXXX crossover.    
...
Hereafter, we refer to this method as the homogeneous configuration. Second, for RPSS the parametrizations
are not tuned, they are randomly generated for each EvoWorker, set independently at the beginning of each run.
Hereafter, we call this approach the heterogeneous configuration. Finally, we perform 30 independent runs
